...
contents.post('/new', async (c) => {
  const { title, text, description, ogpImageUrl, tags, publishKeys } = await c.req.json<{
    title: string;
    text: string;
    description: string;
    ogpImageUrl: string;
  ...
  const content = await contentsService.createContent(title, text, description, ogpImageUrl, tags, publishKeys);
